...
def init():
    ...
    if not key:
        ...
        typer.secho("No ENVKEEP_KEY found. A new key has been generated and written to .env.key.", fg=typer.colors.BLUE)
        typer.secho("Keep .env.key secret! It is required to decrypt your encrypted .env files.", fg=typer.colors.YELLOW, bold=True)
        typer.secho(".env.key has been added to .gitignore automatically.", fg=typer.colors.BLUE)
        typer.secho("You can also set ENVKEEP_KEY in your shell if you prefer:", fg=typer.colors.BLUE)
        typer.secho(f"export ENVKEEP_KEY={new_key}", fg=typer.colors.GREEN)
    else:
        typer.secho("ENVKEEP_KEY found in environment.", fg=typer.colors.GREEN)
    typer.secho("EnvKeep initialized!", fg=typer.colors.GREEN)

@app.command()
def encrypt(file: Path = typer.Option(config.DEFAULT_ENV_FILE, '--file', help=".env file to encrypt")):
    ...
    if not file.exists():
        ...
        raise typer.Exit(code=1)
    output = file.parent / (file.name + '.encrypted' if not file.name.endswith('.encrypted') else file.name)
...
@app.command()
def decrypt(file: Path = typer.Option(config.DEFAULT_ENCRYPTED_FILE, '--file', help="Encrypted file to decrypt")):
    ...
    if not file.exists():
        typer.secho(f"File {file} does not exist.", fg=typer.colors.RED)
        raise typer.Exit(code=1)
    output = file.parent / (file.name.replace('.encrypted', '') if file.name.endswith('.encrypted') else '.env')
    if output.exists() and not utils.prompt_overwrite(output):
        raise typer.Exit(code=1)
    if core.decrypt_file(file, output, key):
        typer.secho(f"Decrypted file written to {output}", fg=typer.colors.GREEN)
...
@app.command()
def restore(backup: Optional[str] = typer.Option(None, '--backup', help="Timestamp of backup to restore")):
    ...
    if utils.safe_copy(backup_file, config.DEFAULT_ENCRYPTED_FILE, overwrite=True):
        typer.secho(f"Restored backup {backup} to {config.DEFAULT_ENCRYPTED_FILE}", fg=typer.colors.GREEN)

@app.command()
def status():
    """Show current project status and recent backups."""
    initialized = config.ENVKEEP_DIR.exists() and config.CONFIG_FILE.exists()
    typer.secho(f"Project initialized: {'Yes' if initialized else 'No'}", fg=typer.colors.GREEN if initialized else typer.colors.RED)
    if initialized:
        meta = utils.read_json(config.CONFIG_FILE) or {}
        typer.secho(f"Project: {meta.get(config.CONFIG_KEYS['project_name'], '-')}", fg=typer.colors.BLUE)
        typer.secho(f"Created: {meta.get(config.CONFIG_KEYS['created_at'], '-')}", fg=typer.colors.BLUE)
        typer.secho(f"Last backup: {meta.get(config.CONFIG_KEYS['last_backup'], '-')}", fg=typer.colors.BLUE)
        backups = meta.get(config.CONFIG_KEYS['backups'], [])
        if backups:
    ...
    key = core.get_key()
    typer.secho(f"Key status: {'Set' if key else 'Not set'}", fg=typer.colors.GREEN if key else typer.colors.RED)
